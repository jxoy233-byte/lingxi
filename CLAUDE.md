...
40. **v0.2.4 约定 —— 应用启动链路鲁棒性 + 重启遮罩 retry**：
    - **Bootstrap 主动停止（模块级取消令牌）**（`main.js:bootstrapSession` + `checkBootstrapCancelled()` + `startup:cancel-bootstrap` handler + `BootstrapView.vue`「停止启动」按钮）：冷启动链路里 `uv sync` / `docker build` / Python 首次加载慢，用户配错 key 想中止时只能干等到底。**解决**：模块级 `let bootstrapSession = { cancelled: false, startedAt }` 取消令牌，`startup:bootstrap` handler 入口置 false / finally 置 null（避免下次启动撞 cancelled 残留立刻抛错崩掉），**每个 await 前调 `checkBootstrapCancelled()` 抛 `BOOTSTRAP_CANCELLED`** 跳出；catch 块识别 `err.code === 'BOOTSTRAP_CANCELLED'` 返 `{ ok: false, cancelled: true }`（不算 error）；`startup:cancel-bootstrap` handler 翻 cancelled + `killChild(backendProcRef, 'backend-cancelled')` + `killTrackedChildren('user-cancel')` 杀背景子进程。**为什么不复用 AbortController**：项目没 signal 透传链（probeXxx / fixXxx 都是 fire-and-forget spawn），AbortController 也只是抛 abort 错误，模块级单例状态更直观，handler 间共享零成本。**Why err.code 而非 message**：catch 分支判 `err.code === 'BOOTSTRAP_CANCELLED'` 比字符串匹配稳，i18n / log 重写不误判。**已知限制（不修）**：`uv sync` 中途 kill shell 杀了但 `uv` worker 可能孤儿（无 process group）；`docker build` 中途 kill 留 build cache（下次复用 layer）；端口 38211 残留由 `killPortIfListening` 在下次 `pickBackendPort` 兜底（500ms wait）。
    - **Restart 遮罩「再重启一次」按钮 + `_restartVersion` race 防护**（`App.vue:.restart-mask` + `handleRestartBackend` + `retriggerRestart`）：重启失误 / 后端没修好时 mask 出现即显示按钮（无延迟），让用户能立刻再触发一次。**三要点**：① **`handleRestartBackend` 去掉 `if (_backendRestarting) return` 早返回** → 让 `retriggerRestart` 可复用本方法启动新一轮；防双触发的责任下移到 `retriggerRestart._restartRetriggering` 标记。② **`_restartVersion` 计数器防 race**：每次调用 `++this._restartVersion`，await 完成后比对当前 version，**不是最新就丢弃结果**（避免旧 promise 在新重启启动后到达覆盖状态）。③ **失败用 `_restartError` inline 展示**（不再 alert）→ 遮罩不消失 + 按钮仍可点，用户不会因 alert 点掉遮罩而失去 retry 入口。**已知限制**：主进程 `restartBackend()` 自身不感知「上次还在 in-flight」→ 多次连点触发并行 kill+start；UI 由 `_restartVersion` 保证只听最后一次结果，最终 backend 健康状态正确。**首次 restart 完成后走 `refreshPage()`（不是 `window.location.reload()`）**：Electron `protocol.handle('file')` 拦截器下 JS 级 reload 偶尔被拦截，`_backendRestarting` 不会被重置 → spinner 遮罩卡住不消失。
    - **Electron 单实例锁（防双开撞端口 + 配置）**（`main.js:gotTheLock` + `second-instance` event）：双击启动图标 / 多终端 `electron .` 同时跑两个实例撞 userData / 端口 38211 / 配置文件写冲突。**解决**：`const gotTheLock = app.requestSingleInstanceLock()`；false → `app.quit() + process.exit(0)`（**`app.quit()` 是异步的，不加 `process.exit(0)` 后面 2000+ 行代码会跑完整个 bootstrap 才退出**）；true → 监听 `second-instance` event 唤醒主进程，把 mainWindow 拉回前台（`isMinimized() → restore` + `!isVisible() → show` + `focus`）。**必须放在动态 import 之后**（configModule `await import('./electron.config.js')`），确保 lock 检查在所有副作用之前。**已知限制**：macOS 第二次启动有时不触发 `second-instance` event（系统行为），但单例锁本身仍生效。
    - **macOS 退出行为符合 HIG（Cmd+Q 退出 / Cmd+W 隐藏窗口 / Cmd+M 最小化到 dock）**（`main.js:window-all-closed` + `createMacosProductionMenu()`）：macOS 标准：Cmd+Q = 退出 app；Cmd+W = 关窗（单窗口 app = 隐藏 app，dock 图标还在可点拉回）；Cmd+M = 最小化到 dock 缩略图（点 dock 拉回）。**解决**：① `window-all-closed` handler 加 `if (IS_MAC) return` —— macOS 上不调 `app.quit()`，否则 backend / VL 模型被杀，用户以为「还在跑」（看 dock 图标）但主进程已退。② `createMacosProductionMenu()` —— 生产模式原本走 `Menu.setApplicationMenu(null)` 把 Cmd+Q / Cmd+M / Cmd+W 都删了；改成 macOS 生产模式装最小菜单（`role: 'minimize'` 默认 Cmd+M 最小化 + `role: 'hide'` 显式绑 Cmd+W 单窗口 app 关窗 + `role: 'hideOthers'` 默认 Cmd+Alt+H + Cmd+Q 退出 + 编辑 submenu），Win/Linux 保持 null。③ `activate` handler 加 `!mainWindow.isVisible() → show()` 兜底（`role: 'hide'` 和 `mainWindow.minimize()` 后点 dock 自动恢复的兜底；OS 不保证 100% 触发 unHide）；`second-instance` 加 `!mainWindow.isDestroyed()` 守卫，Cmd+W 关窗后该对象被销毁，调 `isMinimized()`/`show()` 会抛。**为什么不留 Cmd+H**：单窗口 app 下 Cmd+W 已经等于 Cmd+H 的效果（都是隐藏 app），留 Cmd+H 会和 Cmd+W 功能重复让用户困惑。三方 app（Slack / VSCode / Chrome）都是这个组合——Cmd+W 隐藏 / Cmd+M 最小化 / Cmd+Q 退出。**为什么 Cmd+W 用 `role: 'hide'` 不用 `role: 'close'`**：`close` 会销毁窗口对象（之后只能 recreate），`hide` 只隐藏（dock 图标还在可点拉回），符合「关窗不等于退出」的 macOS 习惯。**不要在 macOS 上调 `app.quit()` 主动退**——交给系统菜单 / dock 右键退出走 `will-quit` cleanup 链（`stopHealthMonitor` + `killTrackedChildren` + `stopLingxiContainers`）。

41. **v0.3 约定 —— 取消上传软删 + 前端 console 清理**：
    - **取消上传软删 upload 阶段产物到 .trash/**（`ChatService/core.py:_move_cached_path_to_trash` + `remove_processed_files` 改造）：原 `os.remove(output.file_path)` 只删 temp 原文件，`cached/{sid}/{filename}_{file_id_short}/` 下的 docling 解析产物（`{file_id}_output/document.md` + images）残留到下次孤立清理。**解决**：① 新增模块级 helper `_move_cached_path_to_trash(abs_path: str) -> Optional[str]`：先 `Path.resolve()` 拿真实路径（防 `..` 绕开），`is_relative_to(CACHED_DIR.resolve())` 校验越界（防 `/etc/passwd`），从解析后路径反推 `session_id` + `rel_path`（不信任 `abs_path` 字面量），`shutil.move` 到 `.trash/{sid}/{YYYYMMDD_HHMMSS}/{rel_path}`（保留目录结构，同秒删不同 file 落到不同子目录不撞名）；失败 `return None` 不抛（cleanup best-effort，遵循 CLAUDE.md「压缩失败不 raise」原则）。② `remove_processed_files` 改为删 `Path(file_path).parent`（即 `{filename}_{file_id_short}/` 整树）而非单个 file——temp 原文件 + docling 输出 + images 一锅端。③ 范围**严格**：不动 `data_analysis/`（那是 AI 跑代码生成的，不属于 upload 阶段；可能多个 file 共享一个 gen_001），每天 11:30 `daily_trash_cleanup` 兜底物理清。**为什么 helper 接受绝对路径而非 `sid + rel_path` 元组**：保持 helper 通用（任何 cached 子路径都能删），调用方决定删哪个层；与 `APIRouter/trash.py:delete_session_file`（HTTP 端点）独立 inline 实现不重复——避免 scope creep，HTTP 端点继续走自己的 inline 校验。**为什么 `Path.resolve()` 必须先于反推 sid**：用户可控 `output.file_path` 字段（前端从 `processed_outputs` 回传，恶意构造 `/etc/passwd` 不会逃出 `is_relative_to(CACHED_DIR)`，但 `..` 字符串里嵌恶意段会被 resolve 吃掉）；先 resolve 再校验是防御纵深。**为什么模块级 logger 而非实例 logger**：helper 不属于 ChatService 类，类层级 `self.logger` 拿不到；`get_logger(__name__)` 复用同一 logger hierarchy。**已知限制**：`shutil.move` 跨 fs 是 copy+delete（Windows 上 C: 跨盘符可能慢）；`shutil.move` 中途崩可能留半截（在原 fs 内移动是原子 rename，但跨 fs 不是）；都接受 —— `.trash/` 物理清本身是兜底。
