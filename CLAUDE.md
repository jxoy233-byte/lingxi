...
- **输入净化**：`_try_compact_react` 调用前先走 `_build_clean_compact_input(context)`，**剥离所有 AIMessage**（含其 `tool_calls` 字段），只保留 imp_ipt 之前的 header、imp_ipt 之后的 `SystemMessage`（旧摘要/warning/中断原因）与多段 `ToolMessage`（原文不合并）。**根因**：react_compact_llm 没绑工具，但与 agent_llm 共用同一个 LLM weights，input 里看到 `tool_calls` 字段会模仿输出 `<tool_call>` 块。剥离 AIMessage 后 input 不再有 tool_calls 结构，模仿概率显著降低；`_try_compact_react` 因此不再调用 `_filter_thinking_content`（input 已干净，filter 跑也是空跑还带 look-behind regex 编译风险）。
- **失败兜底**：长度 [80, 4000] 区间外 / LLM 异常一律 `return None`，context 保持不变。剥离 AIMessage 后旧版"孤立标点兜底 / 残留标签兜底"已不需要（不再依赖 filter）。
...
7.1. **`_filter_thinking_content` 仍负责 agent_node / final_node / imp_ipt 输出**：filter 必须能吃掉 MiniMax-M3 输出的 `[</tool_call>]` / `[<]tool_call[>]` / `[<invoke name="cmd">][<command>...</command>]` 等方括号包装的伪 tool_call 块。**关键顺序**：combined regex（`<tool_call>.*?\[?</?tool_calls?>\]?`，兼容裸闭与方括号包裹闭）必须**先**跑；wrapper 正则放后做孤标记兜底。否则 wrapper 先剥 → tool_call 块找不到闭合 → 留下 78 字符半截垃圾。新增 MiniMax-M3 输出格式时必须同步更新两处 filter（`ChatWorkflow/core.py` + `Memory/core.py`）。
7.2. **`_try_compact_react` 不再调用 filter**：input 已经 `_build_clean_compact_input` 剥离 AIMessage，根本没有 tool_call 残骸可清，filter 跑也是空跑还带 look-behind regex 编译风险。**根因**：react_compact_llm 和 agent_llm 是同一个 LLM weights，看到 input 里的 `tool_calls` 字段会模仿输出 `<tool_call>` 块；剥离 AIMessage 后 input 不再诱导模仿。filter 是其他节点（agent_node / final_node / imp_ipt）的兜底，prompt 是源头。
7.3. **react_compact prompt 显式禁止 tool_call + Few-shot 锚定**：prompt 的"禁止"段必须包含 `<tool_call>` / `[</tool_call>]` / `[<invoke name="cmd">][<command>...]` 等伪 tool_call 格式（即使剥离 AIMessage 后模仿概率降低，仍需在源头约束）；同时配 1 个好例子 + 1 个反例 + 一行点错在哪，few-shot 锚定比单纯禁止清单强得多。
