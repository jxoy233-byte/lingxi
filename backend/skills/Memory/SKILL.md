...
ChatMe 的记忆系统是**两套并行**，互不重叠：

| 系统 | 文件 | 谁写 | 内容 |
|---|---|---|---|
| 叙事总结 | `current.md` | memory LLM（每轮自动） | 对话概要 / 待办事项 / 技术要点 |
| 精确持久 | `facts.md` / `preference.md` / `global/*` | **你（agent LLM）通过 `remember()`** | 精确数值 / 路径 / 业务规则 / 用户偏好 |

**你只需要关注第二列**：`remember()` 写到的 key-value 文件有 upsert 语义（同名 key 替换不重复）+ `global/*` 跨会话共享 + 稳定 key 跨轮可查。

**不要**试图往 current.md 写 —— current.md 由 memory LLM 在后台自动整理（你写不进去，写了也不会被读到）。对话概要 / 待办 / 技术要点已经由它覆盖，不需要你重复记。
...
**不必记** 的通常是：临时状态、调试过程、一次性查询、上一轮已经记过的事实、用户的"嗯嗯 / 好的"这种噪声、当前问题的具体答案（那种让 current.md 对话概要记）。把握不准时宁可少记 —— 写错了比少记更难清理。
