      ...
      <div class="help-dialog" role="dialog" aria-modal="true" aria-labelledby="help-dialog-title" tabindex="-1" ref="dialog">
        ...
        <div class="help-body">
          ...
          <section class="help-section">
            ...
            <h4>启动 / 后端</h4>
            <ul>
              <li><b>首次启动</b>：前置依赖面板检测项目根 / Python / Docker；勾选「后端启动完成后自动进入前端」后，之后每次启动直接显示等待动画并自动进主界面</li>
              <li><b>等待动画 ⇄ 配置面板随时互切</b>：动画上「⚙ 启动配置」/ 右上角 ✕ → 看依赖配置与实时日志；面板右上角 ✕ → 切回等待动画。<b>切换不会中断启动</b></li>
              <li><b>真正放弃启动</b>：等待动画上「⏹ 停止启动」（只有它是取消语义，✕ 只是换视图）</li>
              <li><b>重启后端</b>：<code>/restart</code> 或后端断连横幅的「重新连接」</li>
            </ul>
