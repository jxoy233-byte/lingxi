...
- 更新了对文本和文档文件文件的处理的功能
- 更新了带有搜索引擎功能的graph
- 更新title字段存放在config配置中,updated_at也可以传入这里面，这样应该可以解决没有正常显示对话更新时间距离现在的问题
- 对于传入文件数据，对后端的Conversation进行拓展变量来保存，使得前端每次对话进入仍可以显示上传过的文件(todo) ***使用config传入每一段对话的文件显示，如果传入了的话，否则置为None***
